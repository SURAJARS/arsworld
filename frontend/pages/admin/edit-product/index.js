        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-8 space-y-8">
          ...
          <div>
            ...
            <textarea
              ...
              rows="4"
              className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-accent font-mono text-sm"
              placeholder="https://image1.jpg, https://image2.jpg, https://image3.jpg"
            />
            <p className="text-sm text-gray-600 mt-2">
              💡 Enter image URLs separated by commas. Each URL will be used as a product image.
            </p>
            {formData.images && formData.images.length > 0 && (
              <div className="mt-4">
                <p className="font-semibold mb-2">Preview ({formData.images.length} images):</p>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {formData.images.map((img, idx) => (
                    <div key={idx} className="border rounded-lg overflow-hidden bg-gray-50">
                      ...
                      />
                      <p className="text-xs p-2 truncate text-gray-600">{img}</p>
                    </div>
                  ))}
                </div>
              </div>
